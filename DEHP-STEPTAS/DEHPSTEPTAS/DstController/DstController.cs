...
namespace DEHPSTEPTAS.DstController
{
    ...
    public class DstController : ReactiveObject, IDstController
    {
        ...
        /// <summary>
        /// Backing field for <see cref="StepTasFile"/>
        /// </summary>
        private StepTasFile stepTasFile;

        /// <summary>
        /// Gets or sets the <see cref="StepTasFile"/> instance.
        ///
        /// <seealso cref="Load"/>
        /// <seealso cref="LoadAsync"/>
        /// </summary>
        public StepTasFile StepTASFile
        {
            get => stepTasFile;
            private set => stepTasFile = value;
        }
        ...
        /// <param name="filename">Full path to a STEP-TAS file</param>
        public void Load(string filename)
        {
            ...
            var step = new StepTasFile(filename);

            if (step.HasFailed)
            {
                this.IsLoading = false;

                // In case of error the current StepTasFile is not updated (keep previous)
                this.logger.Error($"Error loading STEP file: { step.ErrorMessage }");

                throw new InvalidOperationException($"Error loading STEP file: { step.ErrorMessage }");  // SPA: Do we continue the excecution of next lines?
            }
            ...
            this.stepTasFile = step;
            this.IsFileOpen = !step.HasFailed;
            this.IsLoading = false;
        }

        /// <summary>
        /// Loads a STEP-TAS file asynchronously.
        /// </summary>
        /// <param name="filename">Full path to a STEP-TAS file</param>
        ...
        /// <summary>
        /// Contains the list of ElementDefinition or ElementUsage that will be transfered
        /// </summary>
        ...
        /// <summary>
        /// Updates mapped parts to <see cref="StepTasRowViewModel.MappingStatusType.Transfered"/>
        /// </summary>
        private void SetMappedToTransferStatus()
        {
            foreach (var item in this.ParameterNodeIds)
            {
                ...
                // This is a solution that permits to do it without deep changes to the data structures.
        ...
        /// </summary>
        /// <param name="part">The <see cref="StepTasRowViewModel"/> data</param>
        /// <returns>A awaitable assert whether the mapping was successful</returns>
        public void Map(StepTasRowViewModel part)
        {
            var parts = new List<StepTasRowViewModel> { part };
        ...
        public async Task Transfer()
        {
            ...
            {
                await this.TransferMappedThingsToHub();  // SPA: Is await usefull here!
            }
            else
            {
                // No transfer to STEP TAS file is available
            }
        }
        ...
        private async Task TransferMappedThingsToHub()
        {
            try
            {
                ...
                {
                    this.SetMappedToTransferStatus();      // Added by SPA
                    this.CleanCurrentMappingOnTransfer();  // Added by SPA
                    return;
                }

                // Step 1: upload file   --> SPA: Is it done even if the last version is the same?
                string filePath = stepTasFile.FileName;
                ...
                // Step 2: update SteptasParameter.source with FileRevision from uploaded file
                ...
                foreach (var item in this.ParameterNodeIds)
                {
                    item.Value.UpdateSource(fileRevision);   // SPA: When is this information transferred to the hub?
                }
                ...
                foreach (var elementBase in this.SelectedThingsToTransfer)
                {
                    if (elementBase is ElementDefinition elementDefinition)
                    {
                        ...
                        foreach (var parameter in elementDefinition.Parameter)   // SPA: ALL PARAMETERS?  Why not only step_tas one?
                        {
                    ...
                    {
                        foreach (var parameterOverride in elementUsage.ParameterOverride)     // SPA: ALL PARAMETERS?  Why not only step_tas one?
                        {
        ...
        public async Task UpdateParametersValueSets()
        {
            ...
            this.UpdateParametersValueSets(transaction, this.SelectedThingsToTransfer.OfType<ElementDefinition>().SelectMany(e => e.Parameter));      // SPA: ALL PARAMETER?  Why not only step_tas one?
            this.UpdateParametersValueSets(transaction, this.SelectedThingsToTransfer.OfType<ElementUsage>().SelectMany(eu => eu.ParameterOverride)); // SPA: ALL PARAMETER?  Why not only step_tas one?
