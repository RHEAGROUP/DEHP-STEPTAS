...
namespace DEHPSTEPTAS.ViewModel.Rows
{
    ...
    /// <summary>
    /// Represents a row of mapped <see cref="ParameterOrOverrideBase"/> and <see cref="StepTasRowViewModel"/>
    /// </summary>
    /// <remarks>
    /// The DEHP STEP-TAS adapter does not have a Hub to STEP mapping capability.
    /// </remarks>
    public class MappingRowViewModel : ReactiveObject
    {
        ...
        public MappingRowViewModel(MappingDirection currentMappingDirection, ParameterBase parameter, StepTasRowViewModel part)
        {
            ...
            this.DstThing = new MappedThing()
            {
                ...
                Value = string.IsNullOrEmpty(part.Name) ? $"STEP Entity {part.Description}" : $"STEP entity {part.Description} used at Rel {part.Name}"
