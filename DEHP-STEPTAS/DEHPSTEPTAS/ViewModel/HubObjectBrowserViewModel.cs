...
namespace DEHPSTEPTAS.ViewModel
{
    ...
    public class HubObjectBrowserViewModel : ObjectBrowserViewModel, IHubObjectBrowserViewModel
    {
        ...
        {
            if (!this.dstHubService.IsSTEPTasParameterType(parameter.ParameterType))
            {
                return;
            }
            ...
            try
            {
                ...
                this.ContextMenu.Add(new ContextMenuItemViewModel(
                    $"Download Associated STEP TAS file to \"{part_name}\" {part_filereference}", "",
        ...
        private void ProcessElementDefinitionRowViewModel(ElementDefinitionRowViewModel row)
        {
            foreach (var irow in row.ContainedRows)
            {
                ...
                if (irow is ParameterRowViewModel parameter && this.dstHubService.IsSTEPTasParameterType(parameter.Thing.ParameterType))
                    {
