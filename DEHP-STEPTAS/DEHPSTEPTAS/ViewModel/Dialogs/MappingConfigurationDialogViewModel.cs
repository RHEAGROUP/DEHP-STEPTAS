...
namespace DEHPSTEPTAS.ViewModel.Dialogs
{
    ...
    public class MappingConfigurationDialogViewModel : ReactiveObject, IMappingConfigurationDialogViewModel, ICloseWindowViewModel
    {
        ...
        /// </summary>
        private StepTasRowViewModel selectedThing;

        /// <summary>
        /// Gets or sets the selected row that represents a <see cref="ReferenceDescription"/>
        /// </summary>
        public StepTasRowViewModel SelectedThing
        {
        ...
        /// <summary>
        /// Sets the target <see cref="StepTasRowViewModel"/> to map
        /// </summary>
        /// <param name="part"></param>
        public void SetPart(StepTasRowViewModel part)
        {
        ...
        /// </summary>
        /// <param name="part">The <see cref="StepTasRowViewModel"/> to be updated</param>
        ...
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, ElementDefinition ed, List<string> warnings)
        {
        ...
        /// <param name="warnings">The container filled with warnings (if there are ones)</param>
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, ElementUsage elementUsage, List<string> warnings)
        {
            if (part.SelectedElementDefinition is null)
            {
        ...
        /// <param name="warnings">The container filled with warnings (if there are ones)</param>
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, Parameter parameter, List<string> warnings)
        {
            if (part.SelectedElementDefinition is null)
            {
        ...
        /// <param name="warnings">The container filled with warnings (if there are ones)</param>
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, ParameterOverride parameterOverride, List<string> warnings)
        {
            if (part.SelectedElementDefinition is null)
            {
        ...
        /// <param name="warnings">The container filled with warnings (if there are ones)</param>
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, Option option, List<string> warnings)
        {
        ...
        /// <param name="warnings">The container filled with warnings (if there are ones)</param>
        private void UpdateSelectionFromMappedConfiguredThing(StepTasRowViewModel part, ActualFiniteState state, List<string> warnings)
        {
        ...
        private void InitializesCommandsAndObservableSubscriptions()
        {
            ...
            // To update dialog content when we click on "Refresh" or "Reload" in "Hub data source" frame 
            CDP4Dal.CDPMessageBus.Current.Listen<SessionEvent>().Where(x => x.Status == SessionStatus.EndUpdate)
                .ObserveOn(RxApp.MainThreadScheduler)
                .Subscribe(x => this.InitializeAvailableProperties());
        }
        ...
        private void UpdateAvailableElementUsages()
        {
            ...
            if (this.SelectedThing?.SelectedElementDefinition is { })
            {
                ...
                {
                    this.logger.Debug($"Available ElementUsage from selected ElementDefinition: \"{eu.Name}\" [{eu.ModelCode()}]");
                }
        ...
        /// <summary>
        /// Updates the target <see cref="StepTasRowViewModel.SelectedParameter"/>s for the <see cref="StepTasRowViewModel.SelectedElementDefinition"/>
        /// </summary>
        //[ExcludeFromCodeCoverage]// ui feedback
        ...
        private void ExecuteContinueCommand()
        {
            try
            {
                ...
                {
                    // When ElementDefinition is not selected, a new one will be created
                ...
                this.CloseWindowBehavior?.Close();    // SPA: TO CLOSE AFTER CLICK ON CONTINUE 
            }
        ...
        private bool CheckMappingDefinition()
        {
            ...
            if (this.SelectedThing.SelectedParameter is null)
            {
                ...
                if (this.SelectedThing.SelectedOption is { })
                {
                    MessageBox.Show($"The target parameter \"{this.dstHubService.FindSTEPTasParameterType()?.Name}\" does not exist at \"{this.SelectedThing.SelectedElementDefinition.Name}\",\nthe selected option \"{this.SelectedThing.SelectedOption.Name}\" will not be used in the mapping",
                        "Mapping information", MessageBoxButton.OK, MessageBoxImage.Information);
                }
