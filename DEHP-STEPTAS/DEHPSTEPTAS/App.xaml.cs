...
namespace DEHPSTEPTAS
{
    ...
    public partial class App
    {
        ...
        public App(ContainerBuilder containerBuilder = null)
        {
            ...
            var splashScreenViewModel = new DXSplashScreenViewModel() { Title = "DEHP STEP-TAS Adapter", Logo = new Uri("pack://application:,,,/Resources/cdplogo3d_48x48.png") };
        ...
        {
            this.logger.Info("--------------------------------------------------------");
            this.logger.Info($"Starting STEP-TAS Adapter {Assembly.GetExecutingAssembly().GetName().Version}");
            this.logger.Info("--------------------------------------------------------");
        }
        ...
        public static void RegisterViewModels(ContainerBuilder containerBuilder)
        {
            ...
            containerBuilder.RegisterType<MappingConfigurationDialogViewModel>().As<IMappingConfigurationDialogViewModel>().SingleInstance();   // SPA: Could we have only oe single instance?   TO CHECK!!!
            ...
            containerBuilder.RegisterType<DstExtractionViewModel>().As<IDstExtractionViewModel>().SingleInstance();
            containerBuilder.RegisterType<UploadCSVViewModel>().As<IUploadCSVViewModel>().SingleInstance();
        }
    }
}
